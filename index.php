<!DOCTYPE html>
<!--
 * A Design by GraphBerry
 * Author: GraphBerry
 * Author URL: http://graphberry.com
 * License: http://graphberry.com/pages/license
-->
<html lang="en">
    <head>
        <meta charset=utf-8>
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>The Sand Pick</title>
        <!-- Load Roboto font -->
        <link href='http://fonts.googleapis.com/css?family=Roboto:400,300,700&amp;subset=latin,latin-ext' rel='stylesheet' type='text/css'>
        <!-- Load css styles -->
        <link rel="stylesheet" type="text/css" href="css/bootstrap.css" />
        <link rel="stylesheet" type="text/css" href="css/bootstrap-responsive.css" />
        <link rel="stylesheet" type="text/css" href="css/style.css" />
        <link rel="stylesheet" type="text/css" href="css/pluton.css" />
        <!--[if IE 7]>
            <link rel="stylesheet" type="text/css" href="css/pluton-ie7.css" />
        <![endif]-->
        <link rel="stylesheet" type="text/css" href="css/jquery.cslider.css" />
        <link rel="stylesheet" type="text/css" href="css/jquery.bxslider.css" />
        <link rel="stylesheet" type="text/css" href="css/animate.css" />
        <!-- Fav and touch icons -->
        <link rel="apple-touch-icon-precomposed" sizes="144x144" href="images/ico/apple-touch-icon-144.png">
        <link rel="apple-touch-icon-precomposed" sizes="114x114" href="images/ico/apple-touch-icon-114.png">
        <link rel="apple-touch-icon-precomposed" sizes="72x72" href="images/apple-touch-icon-72.png">
        <link rel="apple-touch-icon-precomposed" href="images/ico/apple-touch-icon-57.png">
        <link rel="shortcut icon" href="images/ico/favicon.ico">
    </head>
    
    <body>
        <div class="navbar">
            <div class="navbar-inner">
                <div class="container">
                    <a href="#" class="brand">
                        <img src="images/logo.png" width="120" height="40" alt="Logo" />
                        <!-- This is website logo -->
                    </a>
                    <!-- Navigation button, visible on small resolution -->
                    <button type="button" class="btn btn-navbar" data-toggle="collapse" data-target=".nav-collapse">
                        <i class="icon-menu"></i>
                    </button>
                    <!-- Main navigation -->
                    <div class="nav-collapse collapse pull-right">
                        <ul class="nav" id="top-navigation">
                            <li class="active"><a href="#home">Inicio</a></li>
                            <li><a href="#service">Servicios</a></li>
                            <li><a href="#portfolio">Cursos</a></li>
                            <li><a href="#about">Sobre Nosotros</a></li>
                            <li><a href="#contact">Contacto</a></li>
                            <li><a href="/eshop/page/inicio">Tienda en Linea</a></li>
                        </ul>
                    </div>
                    <!-- End main navigation -->
                </div>
            </div>
        </div>
        <!-- Start home section -->
        <div id="home">
            <!-- Start cSlider -->
            <div id="da-slider" class="da-slider">
                <div class="triangle"></div>
                <!-- mask elemet use for masking background image -->
                <div class="mask"></div>
                <!-- All slides centred in container element -->
                <div class="container">
                    <!-- Start first slide -->
                    <div class="da-slide">
                        <h2 class="fittext2">Bienvenido a The Sand Pick</h2>
                        <h4>Tienda de instrumentos musicales</h4>
                        <p>Puedas darte una vuelta por nuestro catálogos, contamos con servicio de envió a domicilio, asesoría, inscripción a cursos para aprender a tocar instrumentos musicales y mucho más!</p>
                        <a href="eshop/login" class="da-link button">Empezar a comprar</a>
                        <div class="da-img">
                            <img src="images/guitar.png" alt="image01" width="320">
                        </div>
                    </div>
                    <!-- End first slide -->
                    <!-- Start second slide -->
                    <div class="da-slide">
                        <h2>¿No sabes que es lo que necesitas?</h2>
                        <h4>Nosotros te damos asesoría</h4>
                        <p>No dudes en preguntar por algo, nuestro personal cuenta con amplios conocimientos para poder brindarte el mejor servicio y así poder ofrecerte lo que satisfaga todas tus necesidades.</p>
                        <a href="#contact" class="da-link button">Contáctanos</a>
                        <div class="da-img">
                            <img src="images/piano.png" width="320" alt="image02">
                        </div>
                    </div>
                    <!-- End second slide -->
                    <!-- Start third slide -->
                    <div class="da-slide">
                        <h2>Nosotros tenemos la Variedad</h2>
                        <h4>¿De verdad no encuentras lo que buscas?</h4>
                        <p>Contamos con toda variedad de instrumentos y accesorios para todo tipo de músico, desde una simple púa para guitarra, hasta los más selectos y raros platillos para completar tu batería, si buscas algo no dudes en buscarlo en nuestra tienda.</p>
                        <a href="eshop/page/inicio" class="da-link button">Ir a la tienda</a>
                        <div class="da-img">
                            <img src="images/bateria.png" width="320" alt="image03">
                        </div>
                    </div>
                    <!-- Start third slide -->
                    <!-- Start cSlide navigation arrows -->
                    <div class="da-arrows">
                        <span class="da-arrows-prev"></span>
                        <span class="da-arrows-next"></span>
                    </div>
                    <!-- End cSlide navigation arrows -->
                </div>
            </div>
        </div>
        <!-- End home section -->
        <!-- Service section start -->
        <div class="section primary-section" id="service">
            <div class="container">
                <!-- Start title section -->
                <div class="title">
                    <h1>¿Qué Somos?</h1>
                    <!-- Section's title goes here -->
                    <p>Más que una tienda de música, Esperamos ofrecerte  otros servicios.</p>
                    <!--Simple description for section goes here. -->
                </div>
                <div class="row-fluid">
                    <div class="span4">
                        <div class="centered service">
                            <div class="circle-border zoom-in">
                                <img class="img-circle" src="images/GuitarIcon.png" alt="service 1">
                            </div>
                            <h3>Instrumentos Musicales</h3>
                            <p>Tenemos a la venta una gran variedad de instrumentos Musicales.</p>
                        </div>
                    </div>
                    <div class="span4">
                        <div class="centered service">
                            <div class="circle-border zoom-in">
                                <img class="img-circle" src="images/assisticon.png" alt="service 2" />
                            </div>
                            <h3>Asesorías</h3>
                            <p>Podemos ayudarte a escoger el mejor producto para tus necesidades</p>
                        </div>
                    </div>
                    <div class="span4">
                        <div class="centered service">
                            <div class="circle-border zoom-in">
                                <img class="img-circle" src="images/musicicon.png" alt="service 3">
                            </div>
                            <h3>Aprende a tocar!</h3>
                            <p>Puedes inscribirte a alguno de nuestros cursos. Aprende a tocar desde guitarra clásica, eléctrica hasta un solo de batería!</p>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <!-- Service section end////////////Test de iframe :v -->
        
        <!-- Portfolio section start -->
        <div class="section secondary-section " id="portfolio">
            <div class="triangle"></div>
            <div class="container">
                <div class=" title">
                    <h1>¿Te interesa alguno de nuestros cursos?</h1>
                    <p>No dudes en asistir a alguno de nuestros cursos, contamos con gran variedad de maestros e instrumentos para aprender a tocar!</p>
                </div>
                <ul class="nav nav-pills">
                    <li class="filter" data-filter="all">
                        <a href="#noAction">Todos</a>
                    </li>
                    <li class="filter" data-filter="guitarra">
                        <a href="#noAction">Guitarra</a>
                    </li>
                    <li class="filter" data-filter="piano">
                        <a href="#noAction">Piano</a>
                    </li>
                    <li class="filter" data-filter="violin">
                        <a href="#noAction">Violin</a>
                    </li>
                    <li class="filter" data-filter="bateria">
                        <a href="#noAction">Bateria</a>
                    </li>
                </ul>
                <!-- Start details for portfolio project 1 -->
                <div id="single-project">
                    <div id="slidingDiv" class="toggleDiv row-fluid single-project">
                        <div class="span6">
                            <img src="images/playguitar.jpg" alt="project 1" />
                        </div>
                        <div class="span6">
                            <div class="project-description">
                                <div class="project-title clearfix">
                                    <h3>Curso de Guitarra Acústica</h3>
                                    <span class="show_hide close">
                                        <i class="icon-cancel"></i>
                                    </span>
                                </div>
                                <div class="project-info">
                                    <div>
                                        <span>Nivel</span>Principiante</div>
                                    <div>
                                        <span>Fecha de Inicio</span>16 de Agosto</div>
                                    <div>
                                        <span>Horario</span>Lunes, Miercoles y Viernes de 4 a 6 pm.</div>
                                    <div>
                                       <a href="#"  class="button button-sp">Inscribirme</a>
                                    </div>
                                </div>
                                <p>Este curso es para las personas que estén dispuestas a empezar en el instrumento, se enseñan conceptos básicos y se da la base para empezar</p>
                            </div>
                        </div>
                    </div>
                    <!-- End details for portfolio project 1 -->
                    <!-- Start details for portfolio project 2 -->
                    <div id="slidingDiv1" class="toggleDiv row-fluid single-project">
                        <div class="span6">
                            <img src="images/playpiano.jpg" alt="project 2">
                        </div>
                        <div class="span6">
                            <div class="project-description">
                                <div class="project-title clearfix">
                                    <h3>Curso de Piano</h3>
                                    <span class="show_hide close">
                                        <i class="icon-cancel"></i>
                                    </span>
                                </div>
                                <div class="project-info">
                                    <div>
                                        <span>Nivel</span>Principiante</div>
                                    <div>
                                        <span>Fecha de Inicio</span>16 de Agosto</div>
                                    <div>
                                        <span>Horario</span>Lunes, Miercoles y Viernes de 4 a 6 pm.</div>
                                    <div>
                                       <a href="#"  class="button button-sp">Inscribirme</a>
                                    </div>
                                </div>
                                <p>Este curso es para las personas que estén dispuestas a empezar en el instrumento, se enseñan conceptos básicos y se da la base para empezar</p>
                            </div>
                        </div>
                    </div>
                    <!-- End details for portfolio project 2 -->
                    <!-- Start details for portfolio project 3 -->
                    <div id="slidingDiv2" class="toggleDiv row-fluid single-project">
                        <div class="span6">
                            <img src="images/playviolin.jpg" alt="project 3">
                        </div>
                        <div class="span6">
                            <div class="project-description">
                                <div class="project-title clearfix">
                                    <h3>Curso de Violin</h3>
                                    <span class="show_hide close">
                                        <i class="icon-cancel"></i>
                                    </span>
                                </div>
                                <div class="project-info">
                                    <div>
                                        <span>Nivel</span>Principiante</div>
                                    <div>
                                        <span>Fecha de Inicio</span>16 de Agosto</div>
                                    <div>
                                        <span>Horario</span>Lunes, Miercoles y Viernes de 4 a 6 pm.</div>
                                    <div>
                                       <a href="#"  class="button button-sp">Inscribirme</a>
                                    </div>
                                </div>
                                <p>Este curso es para las personas que estén dispuestas a empezar en el instrumento, se enseñan conceptos básicos y se da la base para empezar</p>
                            </div>
                        </div>
                    </div>
                    <!-- End details for portfolio project 3 -->
                    <!-- Start details for portfolio project 4 -->
                    <div id="slidingDiv3" class="toggleDiv row-fluid single-project">
                        <div class="span6">
                            <img src="images/playbateria.jpg" alt="project 4">
                        </div>
                        <div class="span6">
                            <div class="project-description">
                                <div class="project-title clearfix">
                                    <h3>Curso de Bateria</h3>
                                    <span class="show_hide close">
                                        <i class="icon-cancel"></i>
                                    </span>
                                </div>
                                <div class="project-info">
                                    <div>
                                        <span>Nivel</span>Principiante</div>
                                    <div>
                                        <span>Fecha de Inicio</span>16 de Agosto</div>
                                    <div>
                                        <span>Horario</span>Lunes, Miercoles y Viernes de 4 a 6 pm.</div>
                                    <div>
                                       <a href="#"  class="button button-sp">Inscribirme</a>
                                    </div>
                                </div>
                                <p>Este curso es para las personas que estén dispuestas a empezar en el instrumento, se enseñan conceptos básicos y se da la base para empezar</p>
                            </div>
                        </div>
                    </div>
                    <!-- End details for portfolio project 4 -->
                    <!-- Start details for portfolio project 5 -->
                    <div id="slidingDiv4" class="toggleDiv row-fluid single-project">
                        <div class="span6">
                            <img src="images/playguitar.jpg" alt="project 5">
                        </div>
                        <div class="span6">
                            <div class="project-description">
                                <div class="project-title clearfix">
                                    <h3>Curso de Guitarra Acústica</h3>
                                    <span class="show_hide close">
                                        <i class="icon-cancel"></i>
                                    </span>
                                </div>
                                <div class="project-info">
                                    <div>
                                        <span>Nivel</span>Intermedio</div>
                                    <div>
                                        <span>Fecha de Inicio</span>16 de Agosto</div>
                                    <div>
                                        <span>Horario</span>Lunes, Miercoles y Viernes de 4 a 6 pm.</div>
                                    <div>
                                       <a href="#"  class="button button-sp">Inscribirme</a>
                                    </div>
                                </div>
                                <p>Este curso esta diseñado para las personas que ya cuentan una base en el instrumento, en este curso se enseñan escalas musicales entre otras técnicas mas avanzadas</p>
                            </div>
                        </div>
                    </div>
                    <!-- End details for portfolio project 5 -->
                    <!-- Start details for portfolio project 6 -->
                    <div id="slidingDiv5" class="toggleDiv row-fluid single-project">
                        <div class="span6">
                            <img src="images/playpiano.jpg" alt="project 6">
                        </div>
                        <div class="span6">
                            <div class="project-description">
                                <div class="project-title clearfix">
                                    <h3>Curso de Piano</h3>
                                    <span class="show_hide close">
                                        <i class="icon-cancel"></i>
                                    </span>
                                </div>
                                <div class="project-info">
                                    <div>
                                        <span>Nivel</span>Intermedio</div>
                                    <div>
                                        <span>Fecha de Inicio</span>16 de Agosto</div>
                                    <div>
                                        <span>Horario</span>Lunes, Miercoles y Viernes de 4 a 6 pm.</div>
                                    <div>
                                       <a href="#"  class="button button-sp">Inscribirme</a>
                                    </div>
                                </div>
                                <p>Este curso esta diseñado para las personas que ya cuentan una base en el instrumento, en este curso se enseñan escalas musicales entre otras técnicas mas avanzadas</p>
                            </div>
                        </div>
                    </div>
                    <!-- End details for portfolio project 6 -->
                    <!-- Start details for portfolio project 7 -->
                    <div id="slidingDiv6" class="toggleDiv row-fluid single-project">
                        <div class="span6">
                            <img src="images/playpiano.jpg" alt="project 7">
                        </div>
                        <div class="span6">
                            <div class="project-description">
                                <div class="project-title clearfix">
                                    <h3>Curso de Piano</h3>
                                    <span class="show_hide close">
                                        <i class="icon-cancel"></i>
                                    </span>
                                </div>
                                <div class="project-info">
                                    <div>
                                        <span>Nivel</span>Avanzado</div>
                                    <div>
                                        <span>Fecha de Inicio</span>16 de Agosto</div>
                                    <div>
                                        <span>Horario</span>Lunes, Miercoles y Viernes de 4 a 6 pm.</div>
                                    <div>
                                       <a href="#"  class="button button-sp">Inscribirme</a>
                                    </div>
                                </div>
                                <p>
                                    Este curso esta diseñado para personas con conocimientos intermedios en el instrumento, se enseña la lectura e interpretación de partituras, así como improvisación y técnicas avanzadas en el instrumento
                                </p>
                            </div>
                        </div>
                    </div>
                    <!-- End details for portfolio project 7 -->
                    <!-- Start details for portfolio project 8 -->
                    <div id="slidingDiv7" class="toggleDiv row-fluid single-project">
                        <div class="span6">
                            <img src="images/playviolin.jpg" alt="project 8">
                        </div>
                        <div class="span6">
                            <div class="project-description">
                                <div class="project-title clearfix">
                                    <h3>Curso de Violin</h3>
                                    <span class="show_hide close">
                                        <i class="icon-cancel"></i>
                                    </span>
                                </div>
                                <div class="project-info">
                                    <div>
                                        <span>Nivel</span>Principiante</div>
                                    <div>
                                        <span>Fecha de Inicio</span>16 de Agosto</div>
                                    <div>
                                        <span>Horario</span>Lunes, Miercoles y Viernes de 4 a 6 pm.</div>
                                    <div>
                                       <a href="#"  class="button button-sp">Inscribirme</a>
                                    </div>
                                </div>
                                <p>Este curso esta diseñado para las personas que ya cuentan una base en el instrumento, en este curso se enseñan escalas musicales entre otras técnicas mas avanzadas</p>
                            </div>
                        </div>
                    </div>
                    <!-- End details for portfolio project 8 -->
                    <!-- Start details for portfolio project 9 -->
                    <div id="slidingDiv8" class="toggleDiv row-fluid single-project">
                        <div class="span6">
                            <img src="images/playguitar.jpg" alt="project 9">
                        </div>
                        <div class="span6">
                            <div class="project-description">
                                <div class="project-title clearfix">
                                    <h3>Curso de Guitarra Eléctrica</h3>
                                    <span class="show_hide close">
                                        <i class="icon-cancel"></i>
                                    </span>
                                </div>
                                <div class="project-info">
                                    <div>
                                        <span>Nivel</span>Principiante</div>
                                    <div>
                                        <span>Fecha de Inicio</span>16 de Agosto</div>
                                    <div>
                                        <span>Horario</span>Lunes, Miercoles y Viernes de 4 a 6 pm.</div>
                                    <div>
                                       <a href="#"  class="button button-sp">Inscribirme</a>
                                    </div>
                                </div>
                                <p>Este curso esta diseñado para personas con conocimientos intermedios en el instrumento, se enseña la lectura e interpretación de partituras, así como improvisación y técnicas avanzadas en el instrumento</p>
                            </div>
                        </div>
                    </div>
                    <!-- End details for portfolio project 9 -->
                    <ul id="portfolio-grid" class="thumbnails row">
                        <li class="span4 mix guitarra">
                            <div class="thumbnail">
                                <img src="images/playguitar.jpg" alt="project 1">
                                <a href="#single-project" class="more show_hide" rel="#slidingDiv">
                                    <i class="icon-plus"></i>
                                </a>
                                <h3>Curso de Guitarra</h3>
                                <p>Principiante</p>
                                <div class="mask"></div>
                            </div>
                        </li>
                        <li class="span4 mix piano">
                            <div class="thumbnail">
                                <img src="images/playpiano.jpg" alt="project 2">
                                <a href="#single-project" class="show_hide more" rel="#slidingDiv1">
                                    <i class="icon-plus"></i>
                                </a>
                                <h3>Curso de Piano</h3>
                                <p>Principiante</p>
                                <div class="mask"></div>
                            </div>
                        </li>
                        <li class="span4 mix violin">
                            <div class="thumbnail">
                                <img src="images/playviolin.jpg" alt="project 3">
                                <a href="#single-project" class="more show_hide" rel="#slidingDiv2">
                                    <i class="icon-plus"></i>
                                </a>
                                <h3>Curso de Violin</h3>
                                <p>Principiante</p>
                                <div class="mask"></div>
                            </div>
                        </li>
                        <li class="span4 mix bateria">
                            <div class="thumbnail">
                                <img src="images/playbateria.jpg" alt="project 4">
                                <a href="#single-project" class="show_hide more" rel="#slidingDiv3">
                                    <i class="icon-plus"></i>
                                </a>
                                <h3>Curso de Bateria</h3>
                                <p>Principiante</p>
                                <div class="mask"></div>
                            </div>
                        </li>
                        <li class="span4 mix guitarra">
                            <div class="thumbnail">
                                <img src="images/playguitar.jpg" alt="project 5">
                                <a href="#single-project" class="show_hide more" rel="#slidingDiv4">
                                    <i class="icon-plus"></i>
                                </a>
                                <h3>Curso de Guitarra</h3>
                                <p>Intermedio</p>
                                <div class="mask"></div>
                            </div>
                        </li>
                        <li class="span4 mix piano">
                            <div class="thumbnail">
                                <img src="images/playpiano.jpg" alt="project 6">
                                <a href="#single-project" class="show_hide more" rel="#slidingDiv5">
                                    <i class="icon-plus"></i>
                                </a>
                                <h3>Curso de Piano</h3>
                                <p>Intermedio</p>
                                <div class="mask"></div>
                            </div>
                        </li>
                        <li class="span4 mix piano">
                            <div class="thumbnail">
                                <img src="images/playpiano.jpg" alt="project 7" />
                                <a href="#single-project" class="show_hide more" rel="#slidingDiv6">
                                    <i class="icon-plus"></i>
                                </a>
                                <h3>Curso de Piano</h3>
                                <p>Avanzado</p>
                                <div class="mask"></div>
                            </div>
                        </li>
                        <li class="span4 mix violin">
                            <div class="thumbnail">
                                <img src="images/playviolin.jpg" alt="project 8">
                                <a href="#single-project" class="show_hide more" rel="#slidingDiv7">
                                    <i class="icon-plus"></i>
                                </a>
                                <h3>Curso de Violin</h3>
                                <p>Intermedio</p>
                                <div class="mask"></div>
                            </div>
                        </li>
                        <li class="span4 mix guitarra">
                            <div class="thumbnail">
                                <img src="images/playguitar.jpg" alt="project 9">
                                <a href="#single-project" class="show_hide more" rel="#slidingDiv8">
                                    <i class="icon-plus"></i>
                                </a>
                                <h3>Curso de Guitarra</h3>
                                <p>Avanzado</p>
                                <div class="mask"></div>
                            </div>
                        </li>
                    </ul>
                </div>
            </div>
        </div>
        <!-- Portfolio section end -->
        <!-- About us section start -->
        <div class="section primary-section" id="about">
            <div class="triangle"></div>
            <div class="container">
                <div class="title">
                    <h1>¿Quiénes somos?</h1>
                    <p>Contamos con gran personal, cada uno especializado para así poder brindarte un mejor servicio.</p>
                </div>
                <div class="row-fluid team">
                    <div class="span4" id="first-person">
                        <div class="thumbnail">
                            <img src="images/brece.jpg" alt="team 1">
                            <h3>Miguel Breceda</h3>
                            <ul class="social">
                                <li>
                                    <a target="_blank" href="https://www.facebook.com/miguel.breceda.12?fref=ts">
                                        <span class="icon-facebook-circled"></span>
                                    </a>
                                </li>
                                <li>
                                    <a target="_blank" href="https://www.facebook.com/The-Sand-Pick-466832693519840/timeline/">
                                        <span class="icon-twitter-circled"></span>
                                    </a>
                                </li>
                                <li>
                                    <a target="_blank" href="https://www.facebook.com/The-Sand-Pick-466832693519840/timeline/"
                                        <span class="icon-linkedin-circled"></span>
                                    </a>
                                </li>
                            </ul>
                            <div class="mask">
                                <h2>Gerente de ventas</h2>
                                <p>Tal vez no sepa de música pero puede ayudarte a realizar una buena compra.</p>
                            </div>
                        </div>
                    </div>
                    <div class="span4" id="second-person">
                        <div class="thumbnail">
                            <img src="images/fabis.jpg" alt="team 1">
                            <h3>Fabian Martinez</h3>
                            <ul class="social">
                                <li>
                                    <a target="_blank" href="https://www.facebook.com/mi.rwarrwa.en.tu.confleis.3">
                                        <span class="icon-facebook-circled"></span>
                                    </a>
                                </li>
                                <li>
                                    <a target="_blank" href="https://www.facebook.com/The-Sand-Pick-466832693519840/timeline/">
                                        <span class="icon-twitter-circled"></span>
                                    </a>
                                </li>
                                <li>
                                    <a target="_blank" href="https://www.facebook.com/The-Sand-Pick-466832693519840/timeline/">
                                        <span class="icon-linkedin-circled"></span>
                                    </a>
                                </li>
                            </ul>
                            <div class="mask">
                                <h2>Maestro de Musica</h2>
                                <p>No existen productos malos, solo productos que no cumplen objetivos.</p>
                            </div>
                        </div>
                    </div>
                    <div class="span4" id="third-person">
                        <div class="thumbnail">
                            <img src="images/beto.jpg" alt="team 1">
                            <h3>Mario Sandoval</h3>
                            <ul class="social">
                                <li>
                                    <a target="_blank" href="https://www.facebook.com/The-Sand-Pick-466832693519840/timeline/">
                                        <span class="icon-facebook-circled"></span>
                                    </a>
                                </li>
                                <li>
                                    <a target="_blank" href="https://www.facebook.com/The-Sand-Pick-466832693519840/timeline/">
                                        <span class="icon-twitter-circled"></span>
                                    </a>
                                </li>
                                <li>
                                    <a target="_blank" href="https://www.facebook.com/The-Sand-Pick-466832693519840/timeline/">
                                        <span class="icon-linkedin-circled"></span>
                                    </a>
                                </li>
                            </ul>
                            <div class="mask">
                                <h2>Barrendero</h2>
                                <p>Alguien tiene que limpiar, no?</p>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="about-text centered">
                    <h3>Sobre Nosotros</h3>
                    <p>The Sand Pick es una empresa 100% hidrocálida con mas de 20 años de experiencia en el mercado de la música, nace como una empresa de venta de guitarras y evoluciona al entretenimiento en general como lo son equipamiento especializado dedicado al montaje e iluminación de eventos de cualquier tamaño, accesorios y venta de instrumentos musicales. The Sand Pick ha sido reconocida como la tienda de instrumentos mas importante de la Zona del Bajío, destacada por su personal altamente calificado, por la amplia variedad de productos y por si constante innovación dirigidas hacer mas placenteras y satisfactoria la experiencia de compra.</p>
                </div>
            </div>
        </div>
        <!-- About us section end -->
        
        <!-- Contact section start -->
        <div id="contact" class="contact">
            <div class="section secondary-section">
             <div class="triangle"></div>
                <div class="container">
                    <div class="title">
                        <h1>Contáctanos</h1>
                        <p>Si tienes alguna duda, comentario o aclaración no dudes en contactarnos</p>
                    </div>
                </div>
                <div class="map-wrapper">
                    <div class="map-canvas" id="map-canvas">Loading map...</div>
                    <div class="container">
                        <div class="row-fluid">
                            <div class="span5 contact-form centered">
                                <h3>Completa el formulario</h3>
                                <div id="successSend" class="alert alert-success invisible">
                                    <strong>Bien!</strong>Tu mensaje se envió correctamente.</div>
                                <div id="errorSend" class="alert alert-error invisible">Error al enviar Mensaje.</div>
                                <form id="contact-form" action="php/mail.php">
                                    <div class="control-group">
                                        <div class="controls">
                                            <input class="span12" type="text" id="name" name="name" placeholder="*Tu nombre..." />
                                            <div class="error left-align" id="err-name">Por favor, ingresa tu nombre.</div>
                                        </div>
                                    </div>
                                    <div class="control-group">
                                        <div class="controls">
                                            <input class="span12" type="email" name="email" id="email" placeholder="* Tu correo electrónico..." />
                                            <div class="error left-align" id="err-email">Por Favor, ingresa un correo electrónico valido.</div>
                                        </div>
                                    </div>
                                    <div class="control-group">
                                        <div class="controls">
                                            <textarea class="span12" name="comment" id="comment" placeholder="* Escribe tus comentarios..."></textarea>
                                            <div class="error left-align" id="err-comment">Por favor, ingresa un mensaje.</div>
                                        </div>
                                    </div>
                                    <div class="control-group">
                                        <div class="controls">
                                            <button id="send-mail" class="message-btn">Enviar Mensaje</button>
                                        </div>
                                    </div>
                                </form>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="container">
                    <div class="span9 center contact-info">
                        <p>400 Luis Donaldo Colosio,Aguascalientes</p>
                        <p class="info-mail">thesandpick@gmail.com</p>
                        <p>+54 449 567 890</p>
                        <p>+54 449 543 850</p>
                        <div class="title">
                            <h3>Síguenos en Redes Sociales</h3>
                        </div>
                    </div>
                    <div class="row-fluid centered">
                        <ul class="social">
                            <li>
                                <a target="_blank" href="https://www.facebook.com/The-Sand-Pick-466832693519840/timeline/">
                                    <span class="icon-facebook-circled"></span>
                                </a>
                            </li>
                            <li>
                                <a target="_blank" href="https://www.facebook.com/The-Sand-Pick-466832693519840/timeline/">
                                    <span class="icon-twitter-circled"></span>
                                </a>
                            </li>
                            <li>
                                <a target="_blank" href="https://www.facebook.com/The-Sand-Pick-466832693519840/timeline/">
                                    <span class="icon-gplus-circled"></span>
                                </a>
                            </li>
                        </ul>
                    </div>
                </div>
            </div>
        </div>
        <!-- Contact section edn -->
        <!-- Footer section start -->
        <div class="footer">
            <p>&copy; 2015 Page by itza & breceda, El contenido de esta pagina es con fines educativos y es ficticio, tema obtenido de <a target="_blank" href="http://www.graphberry.com">Graphberry</a></p>
        </div>
        <!-- Footer section end -->
        <!-- ScrollUp button start -->
        <div class="scrollup">
            <a href="#">
                <i class="icon-up-open"></i>
            </a>
        </div>
        <!-- ScrollUp button end -->
        <!-- Include javascript -->
        <script src="js/jquery.js"></script>
        <script type="text/javascript" src="js/jquery.mixitup.js"></script>
        <script type="text/javascript" src="js/bootstrap.js"></script>
        <script type="text/javascript" src="js/modernizr.custom.js"></script>
        <script type="text/javascript" src="js/jquery.bxslider.js"></script>
        <script type="text/javascript" src="js/jquery.cslider.js"></script>
        <script type="text/javascript" src="js/jquery.placeholder.js"></script>
        <script type="text/javascript" src="js/jquery.inview.js"></script>
        <!-- Load google maps api and call initializeMap function defined in app.js -->
        <script type="text/javascript" src="https://maps.googleapis.com/maps/api/js?callback=initializeMap"></script>
        <!-- css3-mediaqueries.js for IE8 or older -->
        <!--[if lt IE 9]>
            <script src="js/respond.min.js"></script>
        <![endif]-->
        <script type="text/javascript" src="js/app.js"></script>
        <script type="text/javascript">
        var lat = '21.924667'; //Set your latitude.
    var lon = '-102.310665'; //Set your longitude.

    var centerLon = lon - 0.0105;

    var myOptions = {
        scrollwheel: false,
        draggable: false,
        disableDefaultUI: true,
        center: new google.maps.LatLng(lat, centerLon),
        zoom: 15,
        mapTypeId: google.maps.MapTypeId.ROADMAP
    };

    //Bind map to elemet with id map-canvas
    var map = new google.maps.Map(document.getElementById('map-canvas'), myOptions);
    var marker = new google.maps.Marker({
        map: map,
        position: new google.maps.LatLng(lat, lon),
    });

    var infowindow = new google.maps.InfoWindow();

    google.maps.event.addListener(marker, 'click', function () {
        infowindow.open(map, marker);
    });
    infowindow.open(map, marker);</script>
    </body>
    <?php  
        $query="insert into visit values(now())";
        $server = "localhost";
        $usuario = "root";
        $pass = "root";
        $db = "tienda";
        $con =  mysql_connect($server,$usuario,$pass);
        mysql_select_db($db);
        if (!$con) {
            die('No pudo conectarse: ' . mysql_error());
        }
        $result = mysql_query($query);
        if (!$result) {
            die('Error:'.mysql_error());
        }
        mysql_free_result($result);
        mysql_close($con);
    ?>
</html>